...
export default function Home() {
    ...
    const register = async (e) => {
        ...
        const email = String(Email);
        await fetch("/api/sendid", {
            method: "POST",
            body: JSON.stringify(
                {
                    'title':"AllTime ID찾기 안내 메일입니다.",
                    "email": email
                }
            ),
            headers: {
            ...
            .then(async res => {
                if (res.status === 200) {
                    ...
                    await router.push('/findid/success')
                    // toast.update(
                    //     loadid,
                    ...
                    toast.update(
                        ...
                        {
                            type: "error",
                            render: "입력하신 이메일과 일치하는 ID가 없습니다.",
                            autoClose: 5000,
                            isLoading: false,
                            transition: Flip
                        }
            ...
            <main>
                <div className="flex items-center justify-center card min-h-screen bg-base-200">
                    <div className="px-8 py-8 my-24 text-left card bg-base-100 shadow-lg w-1/2">
                        ...
                        <h3 className="text-2xl font-bold text-center">ID찾기</h3>
                        <form>
                                    ...
                                    <div className="relative">
                                        ...
                                        }
                                        {
                                            IsVerify ? (
                                                ...
                                                <button
                                                    className="absolute top-0 right-0 rounded-l-none btn btn-primary"
                                                    type='button' onClick={SendVerify} disabled={IsCountDown}>{IsCountDown?"대기중":"인증 및 ID찾기"}
                                                </button>
                                            )
                                        }
                                    </div>
